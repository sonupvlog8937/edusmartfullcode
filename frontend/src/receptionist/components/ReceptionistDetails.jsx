import { Avatar, Box, Chip, Divider, IconButton, Paper, Skeleton, Snackbar, Alert, Stack, Tooltip, Typography } from "@mui/material";
...
const InfoRow = ({ icon, label, value, copyable, onCopy }) => (
  <Box sx={{ display: "flex", alignItems: "center", px: { xs: 2, md: 3 }, py: 1.5, borderBottom: "1px solid #F1F5F9", "&:last-of-type": { borderBottom: "none" }, gap: 2, "&:hover": { background: "#FAFBFF" } }}>
    ...
    {copyable && value && (<Tooltip title="Copy" arrow><IconButton size="small" onClick={() => onCopy(value)} className="no-print" sx={{ color: "#94A3B8", p: 0.75, "&:hover": { color: "#6366F1", background: "#EEF2FF" } }}><ContentCopyIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>)}
  </Box>
);
...
export default function ReceptionistDetails() {
  ...
  useEffect(() => { axios.get(`${baseUrl}/receptionist/details`).then((r) => setData(r.data.data)).catch(console.error).finally(() => setLoading(false)); }, []);

  const handleCopy = (val) => navigator.clipboard.writeText(val).then(() => setSnack({ open: true, msg: `Copied: ${val}` }));
  const initials = data?.name ? data.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase() : "RC";

  if (loading) return (<Box sx={{ px: { xs: 1.5, md: 3 }, py: 3, maxWidth: 900, mx: "auto" }}><Paper elevation={0} sx={{ borderRadius: 4, border: "1px solid #E0E7FF", overflow: "hidden" }}><Skeleton variant="rectangular" height={120} /><Box sx={{ px: 3, pb: 3 }}><Box sx={{ display: "flex", alignItems: "flex-end", gap: 2, mt: -5, mb: 3 }}><Skeleton variant="circular" width={100} height={100} /><Box sx={{ flex: 1, pb: 1 }}><Skeleton width="50%" height={28} sx={{ mb: 0.5 }} /><Skeleton width="30%" height={18} /></Box></Box>{[1,2,3,4].map((i) => <Skeleton key={i} variant="rounded" height={52} sx={{ mb: 1, borderRadius: 2 }} />)}</Box></Paper></Box>);
  if (!data) return (<Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}><Box sx={{ textAlign: "center" }}><BadgeOutlinedIcon sx={{ fontSize: 56, color: "#CBD5E1", mb: 1 }} /><Typography sx={{ fontWeight: 700, color: "#94A3B8" }}>No data found.</Typography></Box></Box>);
    ...
    <Box sx={{ px: { xs: 1.5, sm: 2, md: 3 }, py: { xs: 2, md: 3 }, background: "#F8FAFF", minHeight: "100vh" }}>
      <Box id="profile-print" sx={{ maxWidth: 860, mx: "auto" }}>
        <Paper elevation={0} sx={{ borderRadius: 4, border: "1px solid #E0E7FF", overflow: "hidden", mb: 2 }}>
          <Box sx={{ height: { xs: 100, md: 130 }, background: "linear-gradient(135deg,#d97706 0%,#f59e0b 50%,#fbbf24 100%)", position: "relative" }}>
            <Box sx={{ position: "absolute", right: -30, top: -30, width: 160, height: 160, borderRadius: "50%", background: "rgba(255,255,255,0.08)" }} />
            <Tooltip title="Print Profile" arrow><IconButton className="no-print" onClick={() => window.print()} sx={{ position: "absolute", top: 12, right: 12, color: "rgba(255,255,255,0.85)", background: "rgba(255,255,255,0.15)", "&:hover": { background: "rgba(255,255,255,0.25)" } }}><PrintIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
          </Box>
          <Box sx={{ px: { xs: 2, md: 3 }, pb: 2, display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "center", sm: "flex-end" }, gap: { xs: 1, sm: 2 }, mt: { xs: "-50px", md: "-56px" } }}>
            <Avatar sx={{ width: { xs: 96, md: 110 }, height: { xs: 96, md: 110 }, fontSize: { xs: 32, md: 38 }, fontWeight: 800, background: "linear-gradient(135deg,#d97706,#f59e0b)", border: "4px solid #fff", boxShadow: "0 4px 20px rgba(217,119,6,0.25)" }}>{initials}</Avatar>
            <Box sx={{ flex: 1, pb: { sm: 1 }, textAlign: { xs: "center", sm: "left" } }}>
              ...
              <Stack direction="row" spacing={1} mt={0.75} justifyContent={{ xs: "center", sm: "flex-start" }} flexWrap="wrap" gap={0.75}>
                <Chip label="Receptionist" size="small" sx={{ height: 22, fontSize: 11, fontWeight: 700, background: "linear-gradient(135deg,#d97706,#f59e0b)", color: "#fff" }} />
      ...
      <Snackbar open={snack.open} autoHideDuration={2000} onClose={() => setSnack((p) => ({ ...p, open: false }))} anchorOrigin={{ vertical: "bottom", horizontal: "center" }}><Alert severity="success" variant="filled" onClose={() => setSnack((p) => ({ ...p, open: false }))} sx={{ borderRadius: 2, fontWeight: 600, fontSize: 13 }}>{snack.msg}</Alert></Snackbar>
    </Box>
  );
}
