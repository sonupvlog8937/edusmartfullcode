...
const NAV = [
  { link: "/receptionist/details",         label: "My Profile",       icon: DashboardIcon },
  { link: "/receptionist/admission-enquiry",label: "Admission Enquiry",icon: PeopleAltIcon },
  { link: "/receptionist/visitor-book",    label: "Visitor Book",     icon: SupportAgentIcon },
  { link: "/receptionist/complaint",       label: "Complaint",        icon: ReportProblemIcon },
  { link: "/receptionist/notice",          label: "Notice",           icon: CampaignIcon },
  { link: "/logout",                       label: "Log Out",          icon: LogoutIcon },
];

export default function Receptionist() {
  const theme = useTheme();
  useDocumentTitle("Receptionist");
  ...
  const isActive = (link) => {
    ...
    if (link === "/receptionist/details") return location.pathname === "/receptionist" || location.pathname === "/receptionist/details";
      ...
      <Drawer variant="permanent" open={open}>
        <DrawerHeader sx={{ justifyContent: "space-between", px: 1 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, pl: 1, flex: 1, minWidth: 0 }}>
            ...
            {open && (<Box sx={{ minWidth: 0 }}><Typography sx={{ fontWeight: 900, letterSpacing: "-0.02em", lineHeight: 1.1 }} noWrap>{APP_NAME}</Typography><Chip size="small" label="Receptionist Panel" sx={{ height: 20, fontWeight: 800, fontSize: "0.7rem", bgcolor: alpha(theme.palette.primary.main, 0.12), color: theme.palette.primary.main, mt: 0.35 }} /></Box>)}
          </Box>
