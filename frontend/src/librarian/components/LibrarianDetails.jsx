...
export default function LibrarianDetails() {
  ...
  useEffect(() => { axios.get(`${baseUrl}/librarian/details`).then((r) => setData(r.data.data)).catch(console.error).finally(() => setLoading(false)); }, []);

  const handleCopy = (val) => navigator.clipboard.writeText(val).then(() => setSnack({ open: true, msg: `Copied: ${val}` }));
  const initials = data?.name ? data.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase() : "LB";
    ...
    <Box sx={{ px: { xs: 1.5, sm: 2, md: 3 }, py: { xs: 2, md: 3 }, background: "#F8FAFF", minHeight: "100vh" }}>
      <Box id="profile-print" sx={{ maxWidth: 860, mx: "auto" }}>
        <Paper elevation={0} sx={{ borderRadius: 4, border: "1px solid #E0E7FF", overflow: "hidden", mb: 2 }}>
          <Box sx={{ height: { xs: 100, md: 130 }, background: "linear-gradient(135deg,#059669 0%,#0891B2 50%,#6366F1 100%)", position: "relative" }}>
          ...
          <Box sx={{ px: { xs: 2, md: 3 }, pb: 2, display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "center", sm: "flex-end" }, gap: { xs: 1, sm: 2 }, mt: { xs: "-50px", md: "-56px" } }}>
            <Avatar sx={{ width: { xs: 96, md: 110 }, height: { xs: 96, md: 110 }, fontSize: { xs: 32, md: 38 }, fontWeight: 800, background: "linear-gradient(135deg,#059669,#0891B2)", border: "4px solid #fff", boxShadow: "0 4px 20px rgba(5,150,105,0.25)" }}>{initials}</Avatar>
            <Box sx={{ flex: 1, pb: { sm: 1 }, textAlign: { xs: "center", sm: "left" } }}>
              ...
              <Stack direction="row" spacing={1} mt={0.75} justifyContent={{ xs: "center", sm: "flex-start" }} flexWrap="wrap" gap={0.75}>
                <Chip label="Librarian" size="small" sx={{ height: 22, fontSize: 11, fontWeight: 700, background: "linear-gradient(135deg,#059669,#0891B2)", color: "#fff" }} />
          ...
          <Box>
            ...
            <InfoRow icon={<LocalLibraryIcon sx={{ fontSize: 18 }} />} label="Salary" value={data.salary ? `₹${data.salary}` : null} />
